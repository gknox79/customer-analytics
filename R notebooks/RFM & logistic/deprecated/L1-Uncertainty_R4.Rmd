...
To illustrate the technique, we are going to use data from eBeer. You can choose where you saved the e-beer.sav file.  We make the working directory wherever this .rmd file is saved.  If you saved it to downloads, it could be "C:\Users\username\Downloads"

```{r message=FALSE, warning=FALSE}
# install.packages(c("foreign", "data.table", "dplyr","VGAM"))
...
class(ebeer$respmail)
levels(ebeer$respmail)
```

Note the resonse variable is a "factor" variable, what R calls a categorical variable, with two levels "no response" and "response".  To calculate the mean, we first have to turn it into a numerical variable, which turns it into 1 and 2.  Then subtract off 1 so that it is 0 and 1.

```{r}
table(as.numeric(ebeer$respmail))

ebeer$respmail<-as.numeric(ebeer$respmail)-1
